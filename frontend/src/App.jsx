        ...
        <header className="sticky top-0 z-50 bg-white shadow-sm border-b border-slate-100">
          <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between gap-6">
            <Link to="/" className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold text-slate-900">Rappi</span>
            </Link>
            
            <nav className="hidden sm:flex items-center gap-8 text-sm font-medium">
              <Link to="/catalog" className="text-slate-700 hover:text-[#FF6B35]">
                Catálogo
              </Link>
              {token && (
                <>
                  <Link to="/cart" className="text-slate-700 hover:text-[#FF6B35]">
                    Carrito
                  </Link>
                  <Link to="/orders" className="text-slate-700 hover:text-[#FF6B35]">
                    Pedidos
                  </Link>
                </>
              )}
            ...
            <div className="flex items-center gap-3">
              {!token ? (
                <Link to="/login" className="px-4 py-2 rounded-lg bg-[#FF6B35] text-white font-semibold text-sm">
                  Ingresar
                </Link>
              ) : (
                <span className="px-4 py-2 text-green-600 font-semibold text-sm">✓ Conectado</span>
              )}
            </div>
          </div>
        </header>
